...
/**
 * What each role may do, as the API enforces it today (issue #24). The
 * client's Roles page renders this table, and the API's
 * `role-permissions.spec.ts` checks it against every controller's
 * `@Roles(...)`, so the page cannot drift from the server:
 *
...
export const PERMISSIONS: readonly Permission[] = [
    ...
    },
    {
        ...
        roles: [Role.Admin],
        routes: [],
        enforcedBy: 'user.policy.ts (canManageUser, canGrantRole)',
    },
];

/** The permissions someone holding only `role` has, in table order. */

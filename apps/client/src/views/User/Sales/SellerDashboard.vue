<template>
    ...
    >
        <div class="max-w-5xl mx-auto w-full space-y-8 mt-2">
            ...
            <div
                v-if="shiftStore.activeShift"
                class="bg-white rounded-2xl border border-slate-200 p-5 shadow-2xs max-w-sm"
            >
                <div
                    class="text-xs font-extrabold uppercase text-slate-400 tracking-wider mb-2"
                >
                    Current Drawer Cash
                </div>
                <div class="text-3xl font-black text-slate-900 font-mono">
                    {{ currency(shiftStore.currentDrawerCash) }}
                </div>
                <p class="text-xs text-slate-500 mt-1">
                    Cashier: {{ shiftStore.activeShift.cashier }}
                </p>
            </div>
        </div>
    </div>
...
import { Banknote, Calculator, ShoppingCart } from '@lucide/vue';

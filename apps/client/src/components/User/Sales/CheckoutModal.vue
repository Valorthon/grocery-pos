<template>
    ...
    >
        <!--
            Enter in a field confirms (issue #22): the form submits through
            the footer's Confirm button, which is disabled until the payment
            is valid. Escape cancels unless the sale is processing (BaseModal
            is not closable then).
        -->
        <form
            :id="formId"
            ref="formEl"
                ...
                >
                    ...
                    <div class="flex flex-wrap gap-1.5">
                        ...
                        <button
                            ...
                            @click="
                                amountTendered =
                                    centavosToPesoInput(roundedUpTotal)
                            "
                        >
                    ...
                    >
                        ...
                        <div class="flex flex-wrap gap-1.5 items-center">
                            <button
                                ...
                                class="px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-xs font-bold text-slate-700 hover:bg-slate-100 active:scale-[0.98]"
                                @click="
                                    splitCashGiven = centavosToPesoInput(bill)
                                "
                            >
                                {{ currency(bill) }}
                            </button>
...
async function selectMethod(next: PaymentType) {
    ...
    await nextTick();
    formEl.value?.querySelector<HTMLElement>('[data-autofocus]')?.focus();
}
